...
import { emitPrismSignal } from "@/lib/hesi-a2/cortex-signals";

describe("PRISM Signals", () => {
  it("dispatches custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ eventType: "question_answered", activityType: "hesi-a2" });
  ...
  it("includes correct event detail", () => {
    ...
    emitPrismSignal({ eventType: "exam_started", value: 1, activityType: "hesi-a2" });
    expect(detail).toEqual({ eventType: "exam_started", value: 1, activityType: "hesi-a2" });
